...
var <%= resource%> = new mongoose.Schema({
  created_at: Date,
  updated_at: Date,
  id: String // Schemas need an explicit id property if you're going to try to insert a custom id
}, {strict:false});
...
function update(req, res) {
  <%= resource%>Model.findOneAndUpdate({_id:req.params.id}, req.body, res.tful);
}
